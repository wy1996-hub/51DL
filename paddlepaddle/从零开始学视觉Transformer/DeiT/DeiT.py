...
class PatchEmbedding(nn.Layer):
    def __init__(self, image_size=224, patch_size=16, in_channels=3, embed_dim=768, dropout=0.):
        ...
        self.position_embedding = paddle.create_parameter(shape=[1, n_patches+2, embed_dim], 
                                                          dtype='float32', 
                                                          default_initializer=nn.initializer.TruncatedNormal(std=.02))
...
class Encoder(nn.Layer):
    ...
    def forward(self, x):
        ...
        return x[:, 0], x[:, 1] # 0:class_token 1:distill_token
...
class DeiT(nn.Layer):
    ...
    def forward(self, x):
        ...
        x = self.classifier(x) # 主要要看encoder的输出是x还是x[:, 0]
        x_distill = self.classifier_distill(x_distill)
        if self.training:
            return x, x_distill
        else:
